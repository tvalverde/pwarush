...
4.  **Design System Adherence:** All UI changes MUST strictly follow the rules defined in the relevant DESIGN.md. Per-app palette and concrete tokens live in `apps/<app>/DESIGN.md`. The shared system (shapes, typography, spacing, semantic tokens without color) will live in `packages/core/DESIGN.md` once extracted in Hito 4.
...
14. **Strategic Delegation:** The primary agent MUST ALWAYS handle planning, complex logic, architectural design, and deep refactoring. Subagents are STRICTLY limited to atomic, mechanical, and well-defined tasks (like running scripts, formatting, or mass simple replacements). Architectural decisions or critical bug resolution MUST NEVER be delegated to a subagent.
