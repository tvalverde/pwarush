...
This document outlines the strict behavioral rules during the development of **PWARUSH** (monorepo with apps Sudokupado, Murdokupado, El Farsante and Neonquiz, plus the shared `@pwarush/core` package).
...
9.  **Version Bumping Rule (Monorepo):** Before each monorepo tag, follow this strict procedure in this exact order:
    ...
    **a) Identify which workspaces changed since the previous tag:**
    ...
    for ws in apps/sudokupado apps/murdokupado apps/elfarsante apps/neonquiz packages/core; do
...
19. **TODO.md Hygiene (truthful backlog):** `TODO.md` MUST stay an accurate mirror of reality, not a write-once dump. **In the same change that resolves a backlog item**, update its entry: mark it `DONE (YYYY-MM-DD)` with a one-line note (or delete it once its history lives in git/commits). NEVER leave a resolved item described as pending — a stale backlog makes you misreport remaining work to the user (e.g. claiming icons are "inherited placeholders" long after own art shipped). Conversely, when new technical debt or a deferred feature appears, record it immediately (rule 10). **Before quoting `TODO.md` as the source of truth for what's pending, VERIFY each claimed-pending item against the actual code** (the file is local-only and easily drifts); correct any drift you find as you go. This complements rule 10 (record pending) and rule 13 (never stage/commit `TODO.md`).
20. **Node Version Stability & Test Integrity:** The project requires **Node 24** (the latest LTS major), pinned as `24` in `.nvmrc` so it tracks the newest 24.x. Other majors are rejected at install time: the root `package.json` sets `engines.node` to `24.x` and `.npmrc` sets `engine-strict=true`, so `npm` fails fast with `EBADENGINE` under e.g. Node 20.x (crashes the toolchain via the `util.styleText` array-argument bug) or Node 25 (leaks unsupported globals like an empty `localStorage` in tests that break the suite). You normally DON'T need a manual `nvm use`: the `Makefile` prepends the nvm-installed Node 24 to `PATH` for every target, and the git hooks load it via `.husky/common.sh`. Only a DIRECT `npm`/`node`/`npx` call (outside make/hooks) uses the shell's default Node — activate nvm there if it isn't 24. Bootstrap a fresh checkout with `make setup`. Furthermore, you MUST NEVER bypass Git hooks (e.g., `git commit --no-verify`) to force a commit when tests fail; test failures invariably point to genuine environmental or code issues that must be addressed.
21. **Neonquiz ADULT Question Phrasing (blind read):** In Neonquiz, `ADULT` (and `BOTH`) questions are read **blind**: the player never sees the four options, only the `questionText` and — after tapping *Reveal* — the single correct answer (`components/AdultQuestionOverlay.tsx`). Therefore any question authored or edited for `ADULT`/`BOTH` audiences MUST be answerable **without** seeing the options: NEVER use phrasings that point at the hidden option list (e.g. "¿Cuál de estos/estas…?", "¿Cuál de los/las siguientes…?", "De estos… ¿cuál…?"), and NEVER use negation-over-a-list phrasings ("¿Cuál … NO …?") that are impossible to answer in the open. Reformulate to a self-contained question with **one unambiguous answer** equal to the correct option (or, for set-valued facts, an enumeration whose full answer lives in the correct option). This constraint does NOT apply to `KID`-only questions (multiple choice, options visible). The regression guard `apps/neonquiz/src/test/adultQuestionPhrasing.test.ts` enforces this over the whole question bank — keep it green.
