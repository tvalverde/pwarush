...
20. **Node Version Stability & Test Integrity:** The project strictly requires Node 24 (LTS), which is enforced via `.nvmrc`. Older versions (e.g., Node 20.12.x) will crash the toolchain due to dependency bugs (e.g., `util.styleText` array arguments), while unstable versions (e.g., Node 25) leak unsupported globals (like an empty `localStorage` in tests) that break the test suite. ALWAYS ensure you are running the exact Node version specified in `.nvmrc`. Furthermore, you MUST NEVER bypass Git hooks (e.g., `git commit --no-verify`) to force a commit when tests fail; test failures invariably point to genuine environmental or code issues that must be addressed.
